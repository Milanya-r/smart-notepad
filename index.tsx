...
// components/Modal.js
// Fix: Changed component signature to accept `props` to resolve issue with `children` prop typing.
const Modal = (props) => {
    ...
    const renderContent = () => {
        ...
        if (hasNotes) {
                ...
                return favoritesByCategory.map(({ category, notes: groupNotes }) => (
                    <div key={category?.id || 'uncategorized'} className="mb-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {/* Fix: Pass props to NoteCard individually to avoid type error with the 'key' prop. */}
                            {groupNotes.map(note => <NoteCard key={note.id} note={note} isSelected={selectedNoteIds.includes(note.id)} categories={categories} onEdit={onEdit} onDelete={onDelete} onToggleFavorite={onToggleFavorite} onTogglePin={onTogglePin} onChangeCategory={onChangeCategory} isSelectionMode={isSelectionMode} onSelect={onSelectNote} onToggleChecklistItem={onToggleChecklistItem}/>)}
                        </div>
                    </div>
                ));
            }
            return (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {/* Fix: Pass props to NoteCard individually to avoid type error with the 'key' prop. */}
                    {notes.map(note => <NoteCard key={note.id} note={note} isSelected={selectedNoteIds.includes(note.id)} categories={categories} onEdit={onEdit} onDelete={onDelete} onToggleFavorite={onToggleFavorite} onTogglePin={onTogglePin} onChangeCategory={onChangeCategory} isSelectionMode={isSelectionMode} onSelect={onSelectNote} onToggleChecklistItem={onToggleChecklistItem}/>)}
                </div>
            );
        }
...
const NoteEditor = ({ note, categories, templates, onSave, onCancel, theme, showToast }) => {
  ...
  const handleExportNote = async (format) => {
    ...
    if (format === 'pdf') {
        ...
        try {
            // Fix: Use declared global `jspdf` instead of `window.jspdf`.
            const { jsPDF } = jspdf;
            const filename = `${sanitizedTitle}.pdf`;
            // Fix: Use declared global `marked` instead of `window.marked`.
            const bodyHtml = marked.parse(content, { gfm: true });
            // Fix: Use declared global `DOMPurify` instead of `window.DOMPurify`.
            ...
            // Fix: Use declared global `html2canvas` instead of `window.html2canvas`.
            const canvas = await html2canvas(printElement.querySelector('.pdf-container'), { scale: 2, useCORS: true, backgroundColor: isDark ? '#0f172a' : '#ffffff' });
    ...
    switch (format) {
        ...
        case 'txt': 
          ...
          // Fix: Use declared global `DOMPurify` instead of `window.DOMPurify`.
          tempDiv.innerHTML = DOMPurify.sanitize(htmlFromMd);
...
const App = () => {
  ...
  // Уровень 1: Локальная проверка напоминаний каждые 30 секунд
  useEffect(() => {
    const intervalId = setInterval(() => {
      if (document.hidden) return; // Не проверять, если вкладка неактивна

      let shouldUpdateState = false;
      const now = Date.now();
      const updatedNotes = notes.map(note => {
          if (note.reminder && note.reminder.nextDueDate && now >= note.reminder.nextDueDate) {
              const nextDueDate = calculateNextDueDate(note.reminder, now);
              
              if (Notification.permission === 'granted') {
                  navigator.serviceWorker.getRegistration().then(reg => {
                      if (reg) {
                          // Fix: Cast options to `any` to allow `vibrate` property.
                          reg.showNotification(note.title, {
                              body: note.content.substring(0, 100),
                              icon: 'icon.svg',
                              badge: 'icon.svg',
                              vibrate: [200, 100, 200],
                              tag: `note-${note.id}-${note.reminder.nextDueDate}`, // Уникальный тег
                          } as any);
                      }
                  });
              }
              ...
              return { ...note, reminder: { ...note.reminder, nextDueDate } };
          }
          return note;
      });
  ...
  const handleEnablePushNotifications = async () => {
      ...
      try {
          ...
          setFcmToken(null);
          showToast('Ошибка при включении Push-уведомлений.', 'error');
      }
  };
  ...
  const handleSaveNote = async (noteData) => {
    ...
    // Уровень 2: Сохранение данных в Firestore для резервного механизма
    if (fcmToken) {
        try {
            const db = firebase.firestore();
            const reminderRef = db.collection('reminders').doc(noteToSave.id);

            if (processedReminder && processedReminder.nextDueDate) {
                await reminderRef.set({
                    token: fcmToken,
                    title: noteToSave.title,
                    content: noteToSave.content.substring(0, 100),
                    sendAt: processedReminder.nextDueDate,
                    reminderData: processedReminder,
                    noteId: noteToSave.id, // Добавим ID заметки для удобства
                });
                console.log('Reminder data saved to Firestore for backup.');
            } else {
                await reminderRef.delete();
                console.log('Reminder data deleted from Firestore.');
            }
        } catch (error) {
            console.error("Error saving reminder to Firestore:", error);
            showToast('Не удалось сохранить резервную копию напоминания.', 'error');
        }
    } else if (processedReminder) {
       showToast('Включите Push-уведомления в настройках, чтобы напоминания работали, когда приложение закрыто.', 'info');
    }
  ...
  const confirmMoveToTrash = async () => {
    ...
    // При перемещении в корзину также удаляем напоминание из Firestore
    const note = notes.find(n => n.id === noteToTrash);
    if (note && fcmToken) {
        try {
            const db = firebase.firestore();
            await db.collection('reminders').doc(note.id).delete();
        } catch (error) {
             console.error("Failed to delete reminder from Firestore on trashing note:", error);
        }
    }
    setNotes(notes.map(n => n.id === noteToTrash ? { ...n, deletedAt: Date.now(), isPinned: false, reminder: null } : n));
...
// Fix: Add a specific type to the templateData parameter to fix property access errors on unknown type.
  ...
  const confirmDeleteSelected = () => {
    ...
    if (view === 'TRASH') {
        ...
        if (lastDigit === 1 && lastTwoDigits !== 11) message = `${count} заметка удалена`;
        else if ([2, 3, 4].includes(lastDigit) && ![12, 13, 14].includes(lastTwoDigits)) message = `${count} заметки удалены`;
        else message = `${count} заметок удалено`;
    } else {
        const now = Date.now();
        const notesToTrash = notes.filter(n => selectedNoteIds.includes(n.id));

        // Удаляем напоминания из Firestore для всех выбранных заметок
        if (fcmToken && notesToTrash.length > 0) {
            const db = firebase.firestore();
            const batch = db.batch();
            notesToTrash.forEach(note => {
                if (note.reminder) {
                    const reminderRef = db.collection('reminders').doc(note.id);
                    batch.delete(reminderRef);
                }
            });
            batch.commit().catch(err => console.error("Failed to batch delete reminders from Firestore", err));
        }

        setNotes(notes.map(n => selectedNoteIds.includes(n.id) ? {...n, deletedAt: now, isPinned: false, reminder: null} : n));
        if (lastDigit === 1 && lastTwoDigits !== 11) message = `${count} заметка перемещена в корзину`;
        else if ([2, 3, 4].includes(lastDigit) && ![12, 13, 14].includes(lastTwoDigits)) message = `${count} заметки перемещены в корзину`;
        else message = `${count} заметок перемещено в корзину`;
    }
  ...
  const confirmDeleteAllActive = () => {
    const now = Date.now();
    const activeNotesToTrash = notes.filter(n => !n.deletedAt);
    
    if(fcmToken && activeNotesToTrash.length > 0) {
      const db = firebase.firestore();
      const batch = db.batch();
      activeNotesToTrash.forEach(note => {
        if (note.reminder) {
          const reminderRef = db.collection('reminders').doc(note.id);
          batch.delete(reminderRef);
        }
      });
      batch.commit().catch(err => console.error("Failed to batch delete all active reminders from Firestore", err));
    }

    setNotes(notes.map(n => !n.deletedAt ? { ...n, deletedAt: now, isPinned: false, reminder: null } : n));
...
if (typeof firebase !== 'undefined' && !firebase.apps.length && firebaseConfig.apiKey !== "YOUR_API_KEY_HERE") {
    ...
    firebase.firestore(); // Инициализируем Firestore
} else {
